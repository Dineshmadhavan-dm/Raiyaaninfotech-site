            ...
            <div>
                @if ($task)
                    <div class="alert alert-info" style="font-size: 1.1em;">
                        ...
                        Creating subtask for task: <span class="text-primary"><strong>{{ $task->task_name }}</strong></span>
                    </div>
                @endif
            </div>
            ...
            <div class="col-lg-10">
                <div class="card border-0 shadow-sm">
                    <div class="card-body p-4">
                        <form action="{{ route('subtaskstore') }}" method="POST" enctype="multipart/form-data"
                            id="subtaskForm">
                            @csrf

                            <!-- Hidden task field -->
                            <input type="hidden" name="stask_task" id="stask_task"
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-md-6">
                                                ...
                                                {{ $task->modulo ? $task->modulo->mod_name : 'N/A' }}<br>
                                                <strong>Project:</strong>
                                                {{ $task->modulo && $task->modulo->project ? $task->modulo->project->pro_name : 'N/A' }}
                                            </div>
                                            <div class="col-md-6">
                                                <strong>Task Deadline:</strong>
                                                {{ \Carbon\Carbon::parse($task->task_deadline)->format('M d, Y') }}<br>
                                                <strong>Priority:</strong> <span
                                                    class="badge {{ $task->priority_class }}">{{ $task->priority_text }}</span><br>
                                                <strong>Access:</strong> {{ $task->task_accessmod ? 'Private' : 'Public' }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text"
                                            ...
                                            id="stask_name" name="stask_name" value="{{ old('stask_name') }}"
                                ...
                                <div class="col-12">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control @error('stask_desc') is-invalid @enderror"
                                            ...
                                            placeholder="Enter subtask description">{{ old('stask_desc') }}</textarea>
                                        @error('stask_desc')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control @error('stask_status') is-invalid @enderror"
                                            ...
                                            <option value="0" {{ old('stask_status') == '0' ? 'selected' : '' }}>Created
                                            </option>
                                            <option value="1" {{ old('stask_status') == '1' ? 'selected' : '' }}>On
                                                Progress</option>
                                            <option value="2" {{ old('stask_status') == '2' ? 'selected' : '' }}>Completed
                                            </option>
                                        </select>
                                        @error('stask_status')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        ...
                                        <div class="border rounded p-3 bg-light">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio"
                                                            name="stask_accessmod" id="access_public" value="0" {{ old('stask_accessmod') == '0' ? 'checked' : '' }}>
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="radio"
                                                            name="stask_accessmod" id="access_private" value="1" {{ old('stask_accessmod') == '1' ? 'checked' : '' }}>
                                                ...
                                                <div class="col-md-4">
                                                    <div class="form-check">
                                                        <input
                                                            ...
                                                            value="1" {{ old('stask_priority') == '1' ? 'checked' : '' }}>
                                                ...
                                                <div class="col-md-4">
                                                    <div class="form-check">
                                                        <input
                                                            ...
                                                            value="2" {{ old('stask_priority') == '2' ? 'checked' : '' }}>
                                                ...
                                                <div class="col-md-4">
                                                    <div class="form-check">
                                                        <input
                                                            ...
                                                            value="3" {{ old('stask_priority') == '3' ? 'checked' : '' }}>
                            ...
                            <div class="row mb-4">
                                <div class="col-12">
                                    <div class="form-group">
                                        ...
                                        @if ($task && count($availableMembers) > 0)
                                            <div class="alert alert-info mb-3">
                                                <i class="bi bi-info-circle me-2"></i>
                                                @if($task->task_member)
                                                    Showing members from task: <strong>{{ $task->task_name }}</strong>
                                                @else
                                                    Showing members from modulo:
                                                    <strong>{{ $task->modulo->mod_name ?? 'N/A' }}</strong>
                                                @endif
                                            </div>
                                            ...
                                            <select class="form-control @error('stask_assignedto') is-invalid @enderror"
                                                ...
                                                @foreach ($availableMembers as $member)
                                                    <option value="{{ $member->emp_id }}" {{ old('stask_assignedto') == $member->emp_id ? 'selected' : '' }}>
                    ...
                    <small class="d-block text-muted">Send subtask details to assigned person</small>
                            ...
                            <div class="form-actions">
                                <button type="submit" class="btn btn-primary">
                                    <i class="bi bi-plus-lg me-2"></i>Create Subtask
                                </button>
                            </div>
                        </form>
                    </div>
...
.form-switch .form-check-input:focus {
    box-shadow: 0 0 0 0.2rem rgba(52, 152, 219, 0.25);
        ...
        /* Reuse the same CSS styles from task create form */
        .card {
            border: none;
            border-radius: 12px;
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            @if(old('stask_assignedto'))
                updateSelectedMemberDisplay('{{ old('stask_assignedto') }}');
            @endif
