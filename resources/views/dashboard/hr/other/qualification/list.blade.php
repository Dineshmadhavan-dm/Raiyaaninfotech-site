...
<x-tabnav>
    ...
    <div class="main-container ">
        ...
        <div class="tab-content">
            <div id="tab4" class="tab-pane {{ request()->routeIs('qualification') ? 'active' : '' }}">
                        ...
                        <div>
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb mb-0 mt-2">
                                    ...
                                    <li class="breadcrumb-item active text-primary" aria-current="page">Qualification
                                    </li>
                                </ol>
                            </nav>
                        </div>
                        ...
                        <div class="col-lg-8">
                            <div class="card border-0 shadow-sm">
                                <div class="card-body p-4">
                                    <form
                                        action="{{ isset($editqualification) ? route('qualificationupdate', $editqualification->qua_id) : route('qualificationpost') }}"
                                        method="post">
                                        @csrf
                                        @if (isset($editqualification))
                                            @method('PUT')
                                        @endif

                                        <div class="form-group mb-3">
                                            <label for="qua_name" class="form-label">Qualification Name</label>
                                            <input type="text" class="form-control" id="qua_name" name="qua_name"
                                                value="{{ isset($editqualification) ? $editqualification->qua_name : old('qua_name') }}">
                                            @error('qua_name')
                                        ...
                                        <div class="d-flex justify-content-end gap-2">
                                            ...
                                            @endif
                                            @can('hrmanage->qualification create')
                                                <button type="submit" class="btn btn-primary px-4" id="submit_form">
                                                    {{ isset($editqualification) ? 'Update' : 'Add' }}
                                                </button>
                                            @endcan
                                        </div>
                                    </form>
                        ...
                        <div class="card-body p-4">
                            <div class="table-responsive">
                                <table id="basic-datatables" class="display table table-bordered table-hover">
                                    ...
                                    <tbody>
                                        @foreach ($qualification as $it)
                                            <tr class="text-center align-middle">
                                                <td class="align-middle">{{ $loop->iteration }}</td>
                                                <td class="align-middle">{{ $it->qua_name }}</td>
                                                <td class="align-middle">
                                                    <div class="d-flex justify-content-center gap-2">

                                                        <a href="{{ route('qualificationedit', $it->qua_id) }}"
                                                        ...
                                                        <form action="{{ route('qualificationdelete', $it->qua_id) }}"
