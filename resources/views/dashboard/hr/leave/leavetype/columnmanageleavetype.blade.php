<x-layout>
    @section('title', 'Manage Leave Type Columns')
    ...
    <div class="container-fluid py-4 px-4 animate__animated animate__fadeIn">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h4 class="mb-0 fw-semibold text-gradient">
                    <i class="bi bi-columns-gap me-2"></i> Manage Leave Type Columns
                </h4>
                <p class="text-muted mb-0 small mt-3">Drag names to reorder them. Check/uncheck to show/hide columns.</p>
            </div>
            <div>
                <a href="{{ route('leavetypeindex') }}" class="btn btn-outline-primary hover-scale">
        ...
        <form action="{{ route('save.leavetype.columns') }}" method="POST" id="columnsForm">
            @csrf
            ...
            <div class="row p-3">
                ...
                <div class="col-md-4 mb-4">
                    <div class="rounded-4 h-100">
                        <div class="card-header bg-light">
                            <h6 class="fw-semibold mb-0">Employee Information</h6>
                        </div>
                        <div class="card-body mt-4">
                            @foreach ($allColumns['employee'] as $key => $label)
                ...
                <div class="col-md-4 mb-4">
                    <div class="rounded-4 h-100">
                        ...
                        <div class="card-body mt-4">
                            @foreach ($allColumns['leave_types'] as $key => $label)
                                <div class="form-check mb-2">
                                    <input class="form-check-input column-toggle" type="checkbox"
                                        name="visible_columns[]" value="{{ $key }}"
                                        id="{{ $key }}Toggle"
                ...
                <div class="col-md-4 mb-4">
                    <div class="rounded-4 h-100">
                        <div class="card-header bg-light">
                            <h6 class="fw-semibold mb-0">Other Information</h6>
                        </div>
                        <div class="card-body mt-4">
                            @foreach ($allColumns['other'] as $key => $label)
                                <div class="form-check mb-2">
                                    <input class="form-check-input column-toggle" type="checkbox"
                                        name="visible_columns[]" value="{{ $key }}"
                                        id="{{ $key }}Toggle"
            ...
            document.getElementById('resetDefaultsBtn').addEventListener('click', function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        fetch('{{ route('reset.leavetype.columns') }}', {
                                method: 'POST',
                                headers: {
